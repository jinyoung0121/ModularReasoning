CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.run --nproc_per_node 2 --master_port=9245 jdev.py \
    --options \
    exp_name internlmxcomposer2_internlm_fps05_min16_stepbystep \
    ...
    video_context datas/IntentQA/internlmxcomposer2_brief_halfframe_val.json \
    llm_type internlm \
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.run --nproc_per_node 2 --master_port=9245 jdev.py \
    ...
    log_freq 10 \
    internlm.max_batch_size 1 \
    video_context datas/NExT-QA/nextqa/internlmxcomposer2_brief_halfframe_val.json \
    llm_type internlm \
    image_vlm_type internlmxcomposer \
    video_vlm_type videollava \
    mode jdev